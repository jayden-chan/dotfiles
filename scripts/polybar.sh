...
if [ "$(hostname)" = "swift" ]; then
    ...
    MONITOR=HDMI-0  polybar --config="$config_path" base   2>&1 | tee -a /tmp/polybar1.log & disown
    MONITOR=DP-0    polybar --config="$config_path" center 2>&1 | tee -a /tmp/polybar2.log & disown
    MONITOR=DVI-D-0 polybar --config="$config_path" base   2>&1 | tee -a /tmp/polybar3.log & disown
fi
